...
def main():
    ...
    parser.add_argument('--levelversion', type=int, default=0, choices=range(1, 127), metavar="[1-127]", 
    ...
    if not hasattr(args, 'songid') or args.songid is None:
        songid_value = get_gmd_value_by_key(gmd_content, 'k35')
        try:
            args.songid = int(songid_value) if songid_value is not None else 0
        except ValueError:
            print(f"Error: songid: '{songid_value}' ")
            return
